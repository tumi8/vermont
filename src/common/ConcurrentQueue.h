 ...
 * types of queues
 */
enum QUEUETYPES {STL, SINGLE, MULTI};
...
class ConcurrentQueue
{
	public:
		 ...
		 */
		ConcurrentQueue(uint32_t qType = MULTI, uint32_t maxEntries = DEFAULT_QUEUE_SIZE, uint32_t spinLockTimeout = DEFAULT_TIMEOUT)
		{
			switch(qType){
				case STL:
			...
			if(2*sizeof(uint32_t) + sizeof(struct timespec) + sizeof(BaseQueue<T>**) > clsize)
		...
		/**
		 * return number of current inserted elements
		 */
