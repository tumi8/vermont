...
extern "C" {
	...
	void msg_intern(char* logtext, const int level, const char* fmt, va_list* args)
	{
		...
		if (level > msg_level) {
			...
			printf("%02d:%02d:%02d.%03ld[%d] %6s", tform->tm_hour, tform->tm_min, tform->tm_sec, (long)(tv.tv_usec/1000), threadid, MSG_TAB[level]);
#else
			...
			if (logtext != NULL) {
#if defined(DEBUG)
				snprintf(logtext, EXCEPTION_MAXLEN, "%02d:%02d:%02d.%03ld[%d] %6s", tform->tm_hour, tform->tm_min, tform->tm_sec, (long)(tv.tv_usec/1000), threadid, MSG_TAB[level]);
#else
