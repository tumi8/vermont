	...
	queue(NULL)
{
	...
	switch(queueType % 10){
		...
		case 2:
			...
			*(queue->queueImp2) = new LockfreeSingleQueueCacheOpt<uint32_t>(queueSize);
			break;
		case 3:
			if(numProducer != 1)
				THROWEXCEPTION("Queue can only handle 1 producer");
			delete queue->queueImp;
			*(queue->queueImp2) = new LockfreeSingleQueueBasic<uint32_t>(queueSize);
			break;
		case 4:
			if(numProducer != 1)
				THROWEXCEPTION("Queue can only handle 1 producer");
			delete queue->queueImp;
			*(queue->queueImp2) = new LockfreeSingleQueueCacheOptLocal<uint32_t>(queueSize);
			break;
		case 5:
			delete queue->queueImp;
			*(queue->queueImp2) = new LockfreeMultiQueue<uint32_t>(queueSize);
			break;
		default:
