...
public:
	...
	void buildAggregator(Rules* rules, uint32_t minBufferTime, uint32_t maxBufferTime, uint8_t hashbits);
	void buildAggregator(char* rulefile, uint32_t minBufferTime, uint32_t maxBufferTime, uint8_t hashbits);
...
protected:
	 ...
	 */
	virtual BaseHashtable* createHashtable(Rule* rule, uint32_t minBufferTime, uint32_t maxBufferTime, uint8_t hashbits) = 0;
	void poll();
	void exporterThread();
