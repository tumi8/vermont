...
void OSFingerprint::detectOS(){
    ...
    if (m_TCP_Options.eol_set  <= 0){
       if (m_IP_TTL  <= 64){
          if (m_IP_TOS  <= 0){
             if (m_TCP_DOFF  <= 1280){ m_OS_Type = "Linux"; }
             if (m_TCP_DOFF  > 1280){
                if (m_TCP_PSH  <= 0){ m_OS_Type = "Linux"; }
                if (m_TCP_PSH  > 0){
                   if (m_TCP_FIN  <= 0){ m_OS_Type = "Mac"; }
                   if (m_TCP_FIN  > 0){ m_OS_Type = "Linux"; }
                   }
                }
             }
          if (m_IP_TOS  > 0){
             if (m_IP_TOS  <= 16){
                if (m_IP_TOS  <= 2){ m_OS_Type = "Linux"; }
                if (m_IP_TOS  > 2){
                   if (m_IP_TOS  <= 8){
                      if (m_TCP_PSH  <= 0){ m_OS_Type = "Linux"; }
                      if (m_TCP_PSH  > 0){ m_OS_Type = "Mac"; }
                      }
                   if (m_IP_TOS  > 8){ m_OS_Type = "Linux"; }
                   }
                }
             if (m_IP_TOS  > 16){ m_OS_Type = "Mac"; }
             }
          }
       if (m_IP_TTL  > 64){ m_OS_Type = "Linux"; }
       }
    if (m_TCP_Options.eol_set  > 0){ m_OS_Type = "Mac"; }
