...
class IpfixRecord
{
	public:
		...
		struct DataTemplateInfo {
			...
			DataTemplateInfo() : fieldCount(NULL), fieldInfo(NULL), userData(NULL), destroyed(false), 
			freePointers(true), preceding(0), dataCount(0), dataInfo(NULL),
			dataLength(0), data(NULL), anonymized (false) {	}
