	  ...
	  statReceivedPackets(0), moduleId(moduleId)
{
	...
	for (std::vector<std::string>::iterator i = endpoints.begin();
			i != endpoints.end(); ++i) {
		...
		for (std::vector<std::string>::iterator j = channels.begin();
				j != channels.end(); ++j) {
...
IpfixReceiverZmq::~IpfixReceiverZmq()
{
	...
	for (std::vector<zsock_t *>::iterator i = zmq_sockets.begin();
			i != zmq_sockets.end(); ++i) {
		zsock_destroy(&(*i));
	}
...
void IpfixReceiverZmq::run()
{
	...
	while (!zsys_interrupted && !exitFlag) {
		...
		for (size_t j = 0; j < num_frames; ++j) {
			...
			mutex.lock();
			for (std::list<IpfixPacketProcessor*>::iterator i =
					packetProcessors.begin();
