...
{
	char buffer[20];
	uint16_t i = IpfixRecord::DataTemplateInfo::getRawPacketFieldIndex(IPFIX_TYPEID_sourceIPv4Address, p);
	...
	i = IpfixRecord::DataTemplateInfo::getRawPacketFieldIndex(IPFIX_TYPEID_destinationIPv4Address, p);
	...
	i = IpfixRecord::DataTemplateInfo::getRawPacketFieldIndex(IPFIX_TYPEID_protocolIdentifier, p);
	uint8_t protocol = *(uint8_t*)(p->netHeader+i);
	snprintf(buffer, 20, "%hhu", protocol);
	msg->setVariable(PAR_PROTOCOL, buffer);
	i = IpfixRecord::DataTemplateInfo::getRawPacketFieldIndex(IPFIX_TYPEID_octetDeltaCount, p);
	...
	if ((protocol & (Packet::TCP|Packet::UDP))>0) {
		i = IpfixRecord::DataTemplateInfo::getRawPacketFieldIndex(IPFIX_TYPEID_sourceTransportPort, p);
		uint16_t srcport = *(uint16_t*)(p->netHeader+i);
		snprintf(buffer, 20, "%hu", srcport);
		msg->setVariable(PAR_SRCPORT, srcport);
		i = IpfixRecord::DataTemplateInfo::getRawPacketFieldIndex(IPFIX_TYPEID_destinationTransportPort, p);
